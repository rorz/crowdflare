...
async function openChatIfPossible() {
...
function activate(context) {
    const run = vscode.commands.registerCommand("crowdflare.crowdsource", async () => {
        ...
        const prompt = cfg.get("crowdsourcePrompt") ?? "";
        const opened = await openChatIfPossible();
        if (!opened) {
            vscode.window.showWarningMessage("Crowdflare: could not open Chat from this build — use Command Palette → Crowdflare: Crowdsource still runs your prompt.");
        }
        if (prompt.length > 0) {
            await vscode.env.clipboard.writeText(prompt);
            vscode.window.showInformationMessage("Crowdflare: Crowdsource prompt copied to clipboard — paste into Chat (⌘V).");
        }
        else {
            vscode.window.showInformationMessage("Crowdflare: Crowdsource — set crowdflare.crowdsourcePrompt in Settings to auto-copy a prompt.");
        }
    });
