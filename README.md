...
- `scripts/discord-helper.mjs` posts polls and reads reaction counts through Discord's REST API
...
- `.cursor/mcp.json.example`: old MCP example if you want to revisit that path later
- `scripts/discord-helper.mjs`: no-dependency Discord helper for the hackathon path
- `DECISIONS.md`: append-only decision log
- `src/extension.ts`: tiny extension that opens chat and copies the run prompt

## Recommended demo flow

1. Copy `.env.example` to `.env` and fill in `DISCORD_TOKEN` and `DISCORD_GUILD_ID`.
...
5. The agent uses `scripts/discord-helper.mjs` to post a two-option Discord poll, waits about 30 seconds, reads the result, logs it, and continues.
